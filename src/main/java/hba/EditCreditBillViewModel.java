...
{
	private Logger logger = Logger.getLogger(EditCreditBillViewModel.class);
	...
	private boolean transferedBill=false;
	private boolean selectIR=false;
	...
	public EditCreditBillViewModel()
	{
		try
		{
			...
			Map map = exec.getArg();
			editBillKey=0;
			//editBillKey=(int) map.get("editBillKey");
			editBillKey=(Integer) map.get("editBillKey");
			String type=(String)map.get("type");
			if(map.get("changeToBill")!=null)
				changeToBill=(Boolean) map.get("changeToBill");
			...
			Window win = (Window)Path.getComponent("/editBillPopUp");
			if(type.equalsIgnoreCase("create"))
			{
				win.setTitle("Add Bill Info");
			}
			else if(type.equalsIgnoreCase("edit"))
			{
				win.setTitle("Edit Bill Info");
			}
			else
			{
				win.setTitle("View Bill Info");
				canModify=false;
				canCreate=false;
			}
			...
			if(editBillKey>0)
			{
				...
				if(objCheque!=null && objCheque.getRecNo()>0)
				{
					...
					if(objCheque.getTransformIR().equalsIgnoreCase("Y")){
						transferedBill=true;
					}else{
						transferedBill=false;
					}
			...
			else
			{
				...
				if(changeToBill)
				{
					...
					int RecNo=(Integer) map.get("itemReceiptKey");
					for(QbListsModel vendorList:lstPayToOrder)
					{
					...
					List<SelectItemReceiptModel> lstIR =data.getItemReceipt(0,null, null, null,0,RecNo);
					getlstItemReceipt(lstIR);
				}
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> init", ex);			
		}
	}
...
//		}
	...
	@Command  
	public void addNewChequePayment(@BindingParam("cmp") Window x)
	{
	...
	{	      
		...
		{			
			...
			{			   								
				...
				obj.setTransformIR(objCheque.getTransformIR());
				...
				if(result>0)
				{
					...
					for (CheckItemsModel item : lstCheckItems) 
					{
						...
						if(item.getSelectedItems()!=null)
							if(item.getpORecNo()>0){
								data.insertItemReceiptLine(item);
							}
					...
					if(editBillKey==0)
					{
						...
						BindUtils.postGlobalCommand(null, null, "refreshParentBill", args);
					}
					else
					{
		...
		{
			logger.error("ERROR in EditCreditBillViewModel ----> saveData", ex);
		}
	}
	...
	{	
		...
		catch (Exception ex) 
		{
			...
			logger.info("error At EditCreditBillViewModel-> updateFixedAssetItemDepreciation>>> " + sw.toString());		
		}	
					...
					Messagebox.show("Selected Item have sub Sub Item(s). Do you want to continue?","Item", Messagebox.YES | Messagebox.NO  , Messagebox.QUESTION,
						...
						public void onEvent(Event evt) throws InterruptedException {
							...
							{		 
								...
								getNewTotalAmount();
								BindUtils.postNotifyChange(null, null, EditCreditBillViewModel.this, "lstCheckItems");
								BindUtils.postNotifyChange(null, null, EditCreditBillViewModel.this, "toatlAmount");
								BindUtils.postNotifyChange(null, null, EditCreditBillViewModel.this, "lblCheckItems");
			...
			for (CheckFAItemsModel item : lstCheckFAItems) 
			{
				if(item.getLineNo()!=type.getLineNo())
				{
					if(type.getSelectedFixedAsset()==item.getSelectedFixedAsset())
					{
						...
						type.setSelectedFixedAsset(null);
						BindUtils.postNotifyChange(null, null, EditCreditBillViewModel.this, "lstCheckFAItems");
						return;
					}
	...
	private boolean validateData(boolean flag)
	{
		...
		if(compSetup.getUsePurchaseFlow().equalsIgnoreCase("Y") && objCheque.getTransformIR().equalsIgnoreCase("N")){
		...
		{	
			for(CheckItemsModel gridData:lstCheckItems)
			{
				...
				{		
					...
					{
						Messagebox.show("Since Item Type is Inventory Please select Site Name for the existing record in Items Tab!","Bill",Messagebox.OK,Messagebox.INFORMATION);
						return false;
					}
					...
					{	 		
						Messagebox.show("Please Enter The Quantity in the Items Tab,Empty Transaction is not allowed !!!","Bill",Messagebox.OK,Messagebox.INFORMATION);
						return false;
					}
				}
			}	
...
//		}
	...
	@Command
	@NotifyChange({"lblExpenses","lblCheckItems","lblCheckFAItems","objCheque","selectedCreditBillTerms","editBillKey","creationdate","labelStatus","selectedAccount","billNo","selectedPaytoOrder","creationdate","billDueDate","toatlAmount","lstCheckItems","lstExpenses","lstCheckFAItems","totalAmount","tempTotalAmount","actionTYpe","transferedBill"})
	public void navigationItemReceipt(@BindingParam("cmp") String navigation)
	{
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> navigationItemReceipt", ex);		
		}
	}
	...
	public void copyFunctinality(@BindingParam("cmp") String navigation)
	{
		try
		{
			if(editBillKey>0)
			{
				...
				if(objCheque!=null && objCheque.getRecNo()>0)
				{
					...
					setLabelExpanseItems();
					setLabelFaItems();
					BindUtils.postNotifyChange(null, null, EditCreditBillViewModel.this, "objCheque");
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> copyFunctinality", ex);		
		}
	}
	...
	public void goToRelatedReport()
	{
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> goToRelatedReport", ex);			
		}
	}
	...
	public void createPdfForPrinting()
	{
		if(validateData(false))
		{
			...
			try {
				...
				logger.error("ERROR in EditCreditBillViewModel ----> createPdfForPrinting", ex);
			}
		}
	}
	...
	public void previewPdfForprintingInvoice()
	{
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> previewPdfForprintingInvoice", ex);			
		}
	}
	...
	class HeaderFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			try {
				...
				logger.error(
						"ERROR in EditCreditBillViewModel class HeaderFooter PDf ----> onEndPage",
						e);
				...
				public void onEvent(Event evt)
					...
					if (evt.getName().equals("onNo")) {
						try {
							...
							logger.error(
									"ERROR in EditCreditBillViewModel ----> CustomerSendEmail",
									ex);
						}
					}
				}
	...
	public void selectItemReceipt() {
		try {
			...
			if(!labelStatus.equalsIgnoreCase("edit")){
				...
				Executions.createComponents("/hba/payments/selectItemReceipt.zul", null, arg);
			}
		} catch (Exception ex) {
			logger.error("ERROR in EditCreditBillViewModel ----> selectItemReceipt",ex);
		}
	}
	
	@GlobalCommand 
	@NotifyChange({"lstCheckItems","totalAmount","totalAmount","objCheque","selectIR","lblCheckItems"})
	public void getlstItemReceipt(@BindingParam("lstItemReceipt") List<SelectItemReceiptModel> lstIR)
	{		
		try
		{
			Calendar c = Calendar.getInstance();
			lstCheckItems=new ArrayList<CheckItemsModel>();
			for(SelectItemReceiptModel ItemReceiptLine:lstIR)
			{ 
				if(ItemReceiptLine.getLevel().equalsIgnoreCase("S")){
					CheckItemsModel obj=new CheckItemsModel();
					obj.setLineNo(lstCheckItems.size()+1);
					for(QbListsModel items:lstGridQBItems)
					{
						if(items.getRecNo()==ItemReceiptLine.getItemRefKey())
						{
							obj.setSelectedItems(items);
							break;
						}
					...
					/*for(QbListsModel gridCutomer:lstGridCustomer)
					{
						if(gridCutomer.getRecNo()==ItemReceiptLine.getEntityRefKey())
						{
							obj.setSelectedCustomer(gridCutomer);
							break;
						}
					...
					obj.setDescription(ItemReceiptLine.getDescription());
					obj.setRecNo(editBillKey);
					obj.setAmount(ItemReceiptLine.getAmount());
					obj.setQuantity((int)ItemReceiptLine.getRemainingQuantity());
					obj.setQuantityInHand((int)ItemReceiptLine.getRemainingQuantity());
					obj.setCost(ItemReceiptLine.getCost());
					obj.setCost1(ItemReceiptLine.getCost());
					obj.setRecivedQuantity((int)ItemReceiptLine.getBilledQuantity());
					obj.setpORecNo(ItemReceiptLine.getpORecNo());
					obj.setpOLineNo(ItemReceiptLine.getLineNo());
					obj.setReadOnly(true);
					lstCheckItems.add(obj);
				}
			}
			getNewTotalAmount();
			objCheque.setTransformIR("Y");
			selectIR=true;
			setLabelCheckItems();
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> getlstItemReceipt", ex);			
		}
	}
	
	@GlobalCommand 
	@NotifyChange({"lstExpenses","totalAmount","totalAmount","objCheque","selectIR","lblExpenses"})
	public void getExplstItemReceipt(@BindingParam("explstItemReceipt") List<ExpensesModel> expenseGrid)
	{		
		try
		{
			lstExpenses=new ArrayList<ExpensesModel>();
		...
		{	
			logger.error("ERROR in EditCreditBillViewModel ----> getExplstItemReceipt", ex);			
		}
	}
	...
	public boolean isTransferedBill() {
		return transferedBill;
	}

	public void setTransferedBill(boolean transferedBill) {
		this.transferedBill = transferedBill;
	}
