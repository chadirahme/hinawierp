...
{
	private Logger logger = Logger.getLogger(EditPurchaseOrderViewModel.class);
	...
	public EditPurchaseOrderViewModel()
	{
		try
		{
			...
			Window win = (Window)Path.getComponent("/purchaseOrderPopUp");
			if(type.equalsIgnoreCase("create"))
			{
				win.setTitle("Add Purchase Order Info");
			}
			else if(type.equalsIgnoreCase("edit"))
			{
				win.setTitle("Edit Purchase Order Info");
			}
			else
			{
				win.setTitle("View Purchase Order Info");
				canModify=false;
				canCreate=false;
			}
		...
		{	
			logger.error("ERROR in EditPurchaseOrderViewModel ----> init", ex);			
		}
	}
					...
					Messagebox.show("Selected Item have sub Sub Items(s). Do you want to continue?","Class", Messagebox.YES | Messagebox.NO  , Messagebox.QUESTION,
						...
						public void onEvent(Event evt) throws InterruptedException {
							...
							{		 
								...
								getNewTotalAmount();
								type.setDecription("");
								BindUtils.postNotifyChange(null, null, EditPurchaseOrderViewModel.this, "lstCheckItems");
								BindUtils.postNotifyChange(null, null, EditPurchaseOrderViewModel.this, "totalAmount");
							}
						}
					...
					Messagebox.show("Selected Class have Sub Class(s). Do you want to continue?","Class", Messagebox.YES | Messagebox.NO  , Messagebox.QUESTION,
						...
						public void onEvent(Event evt) throws InterruptedException {
							...
							{		 
								...
								BindUtils.postGlobalCommand(null, null, "resetGrid", args);
								selectedLstClass=null;
								BindUtils.postNotifyChange(null, null, EditPurchaseOrderViewModel.this, "selectedLstClass");
							}
						}
	...
	{	      
		...
		{			
			...
			if(purchaseRequestKey>0)
			{
				Clients.showNotification("The Purchase Order Has Been Updated Successfully.",
						Clients.NOTIFICATION_TYPE_INFO, null, "middle_center", 10000,true);
				Map args = new HashMap();
				BindUtils.postGlobalCommand(null, null, "refreshParentPurchaseOrder", args);		   	        					        
			}
			else
			{
				Clients.showNotification("The Purchase Order Has Been Created Successfully.",
						Clients.NOTIFICATION_TYPE_INFO, null, "middle_center", 10000,true);
				Map args = new HashMap();
				BindUtils.postGlobalCommand(null, null, "refreshParentPurchaseOrder", args);		   		        					        
			}
		...
		{
			logger.error("ERROR in EditPurchaseOrderViewModel ----> Save", ex);			
		}
	}
	...
	public void navigationPurchaseOrder(@BindingParam("cmp") String navigation)
	{
		...
		{	
			logger.error("ERROR in EditPurchaseOrderViewModel ----> navigationPurchaseOrder", ex);		
		}
	}
	...
	public void copyFunctinality()
	{
		...
		{	
			logger.error("ERROR in EditPurchaseOrderViewModel ----> copyFunctinality", ex);		
		}
	}
	...
	public void goToRelatedReport()
	{
		...
		{	
			logger.error("ERROR in EditPurchaseOrderViewModel ----> goToRelatedReport", ex);			
		}
	}
	...
	public void createPdfForPrinting()
	{
		if(validateData(false))
		{
			...
			try {
				...
				logger.error("ERROR in EditPurchaseOrderViewModel ----> createPdfForPrinting", ex);
			}
		}
	}
	...
	public void previewPdfForprintingInvoice()
	{
		...
		{	
			logger.error("ERROR in EditPurchaseOrderViewModel ----> previewPdfForprintingInvoice", ex);			
		}
	}
	...
	class HeaderFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			try {
				...
				logger.error(
						"ERROR in EditPurchaseOrderViewModel class HeaderFooter PDf ----> onEndPage",
						e);
				...
				public void onEvent(Event evt)
					...
					if (evt.getName().equals("onNo")) {
						try {
							...
							logger.error("ERROR in EditPurchaseOrderViewModel ----> CustomerSendEmail",ex);
						}
					}
				}
			});
