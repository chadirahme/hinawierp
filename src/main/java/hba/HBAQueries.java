...
public class HBAQueries {
	...
	public String GetDefaultSetupInfoQuery() {
		...
		query.append(" BuyItemWithHighCost,UseMinPurchasePrice,MinPurchasePriceRatio,MaxPurchasePriceRatio,UseMaxPurchasePrice,address,companyName,AllowToAddInvSite,countrykey,citykey,telephone,ccemail,fax,usebillable,AllowToSkipPurchaseWorkFlow,ClosingDate,DontSaveWithOutMemo,AllowToChangeTXNNo,AllowToChangeChequeNo,Use_VAT, AllowVATCode_ARAP ");
...
//					query.append(" and " + condition);
	...
	public String addNewChequePaymentQuery(CashModel obj) {
		...
		query.append(" ItemBillNoHide,ItemBillDateHide,AcctForPdc,PrintName,UnitKey,UserID,webUserID,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		query.append(" Values(" 
				+ obj.getRecNo() 
				+ ",'" 
				+ obj.getCheckNo()
	...
	public String addNewCashPaymentQuery(CashModel obj) {
		...
		query.append(" ItemBillNoHide,ItemBillDateHide,PrintName,UnitKey,UserID,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		...
		query.append(" '" + obj.getPrintName() + "' , " + obj.getUnitKey()
				+ " , " + obj.getUserID() + " , " + obj.getVatAmount()+", getdate() , getdate() , 'Y'");
		query.append(" )");
		query.append(" ");
		return query.toString();
	}
	...
	public String updateNewCashPayment(CashModel obj) {
		...
		query = new StringBuffer();
		query.append("Update Checkmast set DATEMODIFIED=getdate(), PvNo='" + obj.getPvNo()
		...
		query.append("ItemClassHide='" + obj.getItemClassHide()
				...
				+ obj.getUserID() + ",editedFromOnline='" + editedFromOnline +"' , VAT_AMOUNT="+obj.getVatAmount()
	...
	public String updateNewChequePayment(CashModel obj) {
		query = new StringBuffer();
		query.append("Update Checkmast set DATEMODIFIED=getdate(), "
				+ "PvNo='" + obj.getPvNo()
				...
				+ " , editedFromOnline='Y'" + " , VAT_AMOUNT="+obj.getVatAmount() +" where recNo=" + obj.getRecNo());
		return query.toString();
	...
	public String addNewBankTransferQuery(BankTransferModel obj) {
		...
		query.append(" Cheque,QBRefNo,QBRefDate,QBStatus,ExpClassHide,ExpMemoHide,ExpBillNoHide,ExpBillDateHide,ItemClassHide,ItemDesHide,");
		query.append(" ItemBillNoHide,ItemBillDateHide,PrintName,UnitKey,UserID,SwiftCode,VAT_AMOUNT,TXNTIME ,DATECREATED,CreatedFromOnline)");
		...
		query.append(" '" + obj.getItemClassHide() + "' , '"
				...
				+ "' , '" + obj.getItemBillDateHide() + "' ,");
		query.append(" '" + obj.getPrintName() + "' , " + obj.getUnitKey()
				+ " , " + obj.getUserID() + " , '" + obj.getSwiftCode() + "'" + " , " + obj.getVatAmount() +", getdate() , getdate() , 'Y' " );
	...
	public String deleteQBVATTransactionQuery(int RecNo,int txnType) {
		query = new StringBuffer();
		query.append(" Delete from QBVATTransaction Where TxnRecNo=" + RecNo + " and TxnType=" + txnType);
		return query.toString();
	}

	public String addQBVATTransactionQuery(int recNo, int txnType, VATCodeModel obj ,int nameRefKey,String nameType,double txnAmount) {
		query = new StringBuffer();
		query.append(" Insert into QBVATTransaction ( TxnType,TxnRecNo,TxnVat_Key ,TxnQBItem_Key,TxnVat_Amount,NameRef_Key,NameType,TxnCalculated_Amount,TxnAmount ,TxnStatus)");
		query.append(" values(" + txnType +"," + recNo +","+ obj.getVatKey()+","+obj.getVatServiceItem()+","+obj.getVatPercentage()+","+nameRefKey+", '" + nameType+"' ," + obj.getVatCalcualtedFromAmt() );
		query.append("," + txnAmount + ", 'A' ");
		query.append(" )");
		return query.toString();
	}


	public String getQBVATTransactionQuery(int recNo) {
		query = new StringBuffer();
		query.append(" Select CheckExpense.Vat_Key,Vat_ServiceItem,Sum(Vat_ExpAmount) As VatItemAmount,Sum(CheckExpense.Amount) As VatCalcualtedFromAmt,'EXP' As VatMemo From CheckExpense  Inner Join VATCodeList On CheckExpense.Vat_Key=VATCodeList.Vat_Key " +
				" Where  RecNo  = " + recNo);
		query.append(" Group By CheckExpense.Vat_Key,Vat_ServiceItem");

		query.append(" Union Select  CheckItems.Vat_Key,Vat_ServiceItem,Sum(Vat_ItemAmount) As VatItemAmount,Sum(CheckItems.Amount) As VatCalcualtedFromAmt,'ITM' As VatMemo From  CheckItems Inner Join VATCodeList On  CheckItems.Vat_Key=VATCodeList.Vat_Key " +
				" Where RecNo  = " + recNo);
		query.append(" Group By  CheckItems.Vat_Key,Vat_ServiceItem");
		query.append(" Union Select  CheckFAItems.Vat_Key,Vat_ServiceItem,Sum(Vat_FAAmount) As VatItemAmount,Sum(CheckFAItems.Amount) As VatCalcualtedFromAmt,'FA' As VatMemo From  CheckFAItems  Inner Join VATCodeList On  CheckFAItems.Vat_Key=VATCodeList.Vat_Key ");
		query.append(" Where  RecNo=" +recNo);
		query.append(" Group By  CheckFAItems.Vat_Key,Vat_ServiceItem Order By Vat_Key");

		return query.toString();
	}

	public String getCashInvoiceQBVATTransactionQuery(int recNo) {
		query = new StringBuffer();
		query.append(" Select SalesReceiptLine.Vat_Key,Vat_ServiceItem,Sum(Vat_ItemAmount) As VatItemAmount,Sum(SalesReceiptLine.Amount) As VatCalcualtedFromAmt From SalesReceiptLine " +
				" Inner Join VATCodeList On SalesReceiptLine.Vat_Key=VATCodeList.Vat_Key " +
				" Where  RecNo  = " + recNo);
		query.append(" Group By SalesReceiptLine.Vat_Key,Vat_ServiceItem");

		return query.toString();
	}

	public String getCreditInvoiceQBVATTransactionQuery(int recNo) {
		query = new StringBuffer();
		query.append(" Select InvoiceLine.Vat_Key,Vat_ServiceItem,Sum(Vat_ItemAmount) As VatItemAmount,Sum(InvoiceLine.Amount) As VatCalcualtedFromAmt From InvoiceLine " +
				" Inner Join VATCodeList On InvoiceLine.Vat_Key=VATCodeList.Vat_Key " +
				" Where  RecNo  = " + recNo);
		query.append(" Group By InvoiceLine.Vat_Key,Vat_ServiceItem");
	...
	public String addNewExpenseQuery(ExpensesModel objExpenses, int RecNo) {
		...
		query.append(" Insert into CheckExpense (RecNo,AccountKey,Amount,[Memo],CustKey,ClassKey,[LineNo],BillNO,BillDate,FaItemKey,AccountType,billable,Vat_KEY,Vat_ExpAmount) ");
	...
	public String GetQBItemsQuery() 
	{
		...
		query.append(" DescriptionAR, QuantityOnHand,SalesPrice,averagecost,ClassKey,PurchaseVATKey");
	...
	public String getDefaultSetUpInfoForCashInvoice() {
		...
		query.append("MinSellingPriceRatio,UseMaxSellingPrice,MaxSellingPriceRatio,ShowAvgCost,SaveINVQty,UseSalesRepCommission,canExceedCreditLimit,address,companyName,AllowToAddInvSite,countrykey,citykey,telephone,ccemail,fax,PVSERIALNOS,"
				+ "UseSalesFlow,usePurchaseFlow,AllowToSkipSalesWorkFlow,AllowToSkipPurchaseWorkFlow,ClosingDate,DontSaveWithOutMemo,Use_VAT, AllowVATCode_ARAP");
		if (dbUser.getMergedDatabse().equalsIgnoreCase("Yes")) {
			query.append(" From companySettings");
		} else {
			query.append(" From COMPSETUP");
	...
	public String addNewCashInvoiceQuery(CashInvoiceModel obj, int webUserID) {
		...
		query.append("CustomerSalesTaxCodeRefKey,Other,Amount,QuotationRecNo,SendViaReffKey,CustomField1,CustomField2,CustomField3,CustomField4,CustomField5,status,DescriptionHIDE,QtyHIDE,ClassHIDE,RateHIDE,transformD,webUserID,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		...
		query.append(" '" + obj.getCustomField2() + "' , '"
				...
				+ obj.getRateHIDE() + "' , '"
						+ obj.getTransformD()+ "'," + webUserID + "," + obj.getVatAmount() + ", getdate() , getdate() , 'Y' ");
		query.append(" )");
		query.append(" ");
		return query.toString();
	...
	public String updateCashInvoiceQuery(CashInvoiceModel obj, int webUserID) {
		...
		// DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		query = new StringBuffer();
		query.append("Update SalesReceipt set DATEMODIFIED= getdate(), CustomerRefKey="
	...
	public String addNewCreditInvoiceQuery(CashInvoiceModel obj, int webUserID) {
		...
		query.append("CustomerSalesTaxCodeRefKey,Other,Amount,QuotationRecNo,SendViaReffKey,CustomField1,CustomField2,CustomField3,CustomField4,CustomField5,status,invoice_source,DescriptionHIDE,QtyHIDE,ClassHIDE,RateHIDE,transformD,webUserID,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		query.append(" Values(" + obj.getRecNo() + ","
				+ obj.getCustomerRefKey() + "," + obj.getClassRefKey() + ","
		...
		query.append(" '" + obj.getCustomField2() + "' , '"
				...
				+ "' , '" + obj.getQtyHIDE() + "' , '" + obj.getClassHIDE()
				+ "' , '" + obj.getRateHIDE() + "','"+ obj.getTransformD() + "'," + webUserID + ","+obj.getVatAmount()+", getdate() , getdate() , 'Y' ");
		query.append(")");
		query.append(" ");
		return query.toString();
	...
	public String updateCreditInvoiceQuery(CashInvoiceModel obj, int webUserID) {
		...
		String invoiceSource = "CMS";
		query = new StringBuffer();
		query.append("Update Invoice set DATEMODIFIED=getdate() , CustomerRefKey="
				+ obj.getCustomerRefKey() + ",ClassRefKey="
	...
	public String addNewCreditMemoQuery(CashInvoiceModel obj, int webUserID) {
		...
		query.append("Amount,MemoHide,DescriptionARHide,DescriptionHIDE,QtyHIDE,RateHIDE,ClassHIDE,status,CreditMemo_Source,UserID,DATECREATED,TXNTIME,VAT_AMOUNT,ONLINEFLAG,WEBUSERID,CreatedFromOnline)");
		...
		query.append(obj.getAmount()+ ", 'N', 'N','N','N','N','N','A', 'CMS'," + webUserID + ", getdate() , getdate() , " + obj.getVatAmount() + ", 'Y'," + webUserID + ", 'Y' ");
		query.append(")");
		query.append(" ");
		return query.toString();
	...
	public String updateCreditMemoQuery(CashInvoiceModel obj, int webUserID) {
		...
		query.append("ModifiedUserID=" + webUserID + ",DATEMODIFIED=getdate() , EditedFromOnline='Y' , VAT_AMOUNT= " + obj.getVatAmount()
				 + "  where recNO=" + obj.getRecNo());
		return query.toString();
	...
	public String addNewCashInvoiceGridItemsQuery(CashInvoiceGridData obj,
		...
		if(obj.getSelectedVatCode()!=null)
			query.append(", "  + obj.getSelectedVatCode().getVatKey() + ", "  + obj.getVatAmount()+ ", " + obj.getUnitPriceWithVAT());
		else
			query.append(",0,0,0");
	...
	public String getCashInvoiceSalesReport(Date fromDate, Date toDate,
		...
		query.append("SalesReceipt.Amount As InvAmount,SalesReceipt.VAT_AMOUNT,SalesReceipt.CheckNo,SalesReceipt.Memo AS Notes, ");
	...
	public String getCreditInvoiceReport(Date fromDate, Date toDate,
		...
		query.append(" Invoice.TxnDate As InvoiceDate,Invoice.DueDate As InvoiceDue,Invoice.status,Terms.Name AS TermName,Invoice.Amount As InvAmount,Invoice.VAT_AMOUNT,Invoice.AmountPaid As PaidAmount,Invoice.Invoice_Source, ");
	...
	public String saveQuotation(CashInvoiceModel obj, int webUserID) {
		...
		query.append("ShipAddress2,ShipAddress3,ShipAddress4,ShipAddress5,ShipAddressCity,ShipAddressState,ShipAddressPostalCode,ShipAddressCountry,ShipAddressNote,");
		query.append("IsPending,PONumber,TermsRefKey,DueDate,SalesRefKey,FOB,ShipDate,ShipMethodRefKey,ItemSalesTaxRefKey,Memo,CustomerMsgRefKey,IsToBePrinted,IsToEmailed,IsTaxIncluded,");
		query.append("CustomerSalesTaxCodeRefKey,Other,Amount,SendViaReffKey,CustomField1,CustomField2,CustomField3,CustomField4,CustomField5,status,remindflag,reminddate,reminddays,attachment,DescriptionHIDE,QtyHIDE,RateHIDE,LetterTemplate,ShipToAddress,webUserID,VAT_AMOUNT,DATECREATED,CreatedFromOnline)");
		query.append(" Values(" + obj.getRecNo() + ",'" + obj.getTxnId()
		...
		query.append(", " + obj.getRemindMedays() + ",'" + obj.getAttachemnet()
				...
				+ "','" + obj.getShipToAddress() + "'," + webUserID +"," + obj.getVatAmount() + " , getdate() , 'Y' ");
		query.append(")");
		query.append(" ");
		return query.toString();
	...
	public String updateQuotation(CashInvoiceModel obj, int webUserID) {
		// DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		query = new StringBuffer();
		query.append("Update QUOTATION set DATEMODIFIED=getdate(), TxnID='" + obj.getTxnId()
	...
	public String addNewItemReceipt(ItemReceiptModel obj) {
		// DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		query = new StringBuffer();
		query.append("Insert into IRMAST (RecNo,IRNo,IRNoLocal,IRDate,APAccountKey,VendorKey,Amount,Status,[Memo],");
		query.append("QBStatus,ItemClassHide,ItemDesHide,ItemBillNoHide,ItemBillDateHide,PrintName,IRSource,webUserId,transformPO,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		query.append(" Values(" + obj.getRecNo()
		...
		query.append(" '" + obj.getPrintName() + "','" + obj.getIrsource()
				+ "'," + obj.getWebUserId()+ ",'" + obj.getTransformPO()+"'" +","+obj.getVatAmount() + ", getdate() , getdate() , 'Y' ");
		query.append(" )");
		query.append(" ");
		return query.toString();
	}
	...
	public String updateExistingItemReceipt(ItemReceiptModel obj) {
		...
		String editedFromOnline = "Y";
		query = new StringBuffer();
		query.append("Update IRMAST set DATEMODIFIED=getdate() ,IRNo='"
	...
	public String addNewPurchaseOrder(PurchaseRequestModel obj) {
		query = new StringBuffer();
		query.append("Insert into PurchaseOrder (Rec_No,RefNumber,TxnDate, VendorRefKey, Address, EntityRefKey, ShipTo, ClassRefKey, TotalAmount, [Memo], Status,Source,webUserId,TransformMR,isfullyreceived,VAT_AMOUNT,TXNTIME,DATECREATED,CreatedFromOnline)");
		query.append(" Values(" + obj.getRecNo() 
				+ ",'" + obj.getRefNUmber()
				+ "','" + sdf.format(obj.getTxtnDate()) 
				+ "',"+ obj.getVendorRefKEy() 
	...
	public String updateExistingPurchaseOrder(PurchaseRequestModel obj) {
		query = new StringBuffer();
		query.append("Update PurchaseOrder set DATEMODIFIED=getdate() , RefNumber='"	+ obj.getRefNUmber()
				+ "',TxnDate='"	+ sdf.format(obj.getTxtnDate()) 
				+ "',VendorRefKey="	+ obj.getVendorRefKEy() 
				+ ",Address='" + obj.getAdress()
				+ "',EntityRefKey=" + obj.getEntityRefKey() 
	...
	public String addNewJournalVoucherQuery(JournalVoucherModel obj, int webUserID) {
		query = new StringBuffer();
		query.append("Insert into JournalEntry(Rec_no,TxnNumber,TxnDate,RefNumber,QBStatus,Status,TotalAmount,PreTerRecNo,ReleaseRecNo,ContractRecNo,Notes," +
				"RevisedRecNo," +
				"RVDifferedIncomeFlag,PVAdvancePaymentFlag,webuserid,DATECREATED ,TXNTIME ,CreatedFromOnline) values(");
		...
		if(obj.getTotalAmount()!=0){
			query.append(","+obj.getTotalAmount());
		}else{
			query.append(",0");
		}
		query.append(",0,0,0");
		...
		query.append(",0");
		query.append(",'N','N',"+webUserID+" , getdate(), getdate(), 'Y' )");
		return query.toString();
	}

	public String updateJournalVoucherQuery(JournalVoucherModel obj, int webUserID) {
		query = new StringBuffer();
		query.append("Update JournalEntry set DATEMODIFIED=getdate(), EditedFromOnline='Y' , TxnNumber='"+obj.getTxnNumber()+"'");
		if(obj.getTxnDate()!=null)
	...
	public String updateQuotationStatus(int quotationKey,String status,String statusDesc) {
		query = new StringBuffer();
		query.append("Update Quotation set DATEMODIFIED=getdate(), " );
	...
	public String updateBankTransfer(BankTransferModel obj, int webUserID) {
		...
		query.append(",Amount  ="+obj.getAmount());
		query.append(",VAT_AMOUNT  ="+obj.getVatAmount());
	...
	public String getVatCodeQuery() {
		query = new StringBuffer();
		query.append(" SELECT * from VATCodeList Where IsActive='Y' order by VAT_CODE");
		return query.toString();
	}
