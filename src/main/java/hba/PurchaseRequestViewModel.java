...
{
	private Logger logger = Logger.getLogger(PurchaseRequestViewModel.class);
	...
	//use this to bind combo box inside grid in case not POS
	...
	public PurchaseRequestViewModel()
	{
		try
		{
			...
			{
				lstQBItemsForGrid=lstGridQBItems;
			}
			...
			if(purchaseRequestKey>0)
			{
				...
				if(objCash!=null)
				{
					if(objCash.getStatus().equalsIgnoreCase("A")){
						...
						{
							if(data.checkMRIsFULLYRECEIVED(purchaseRequestKey)){
					...
					requiredDate=df.parse(sdf.format(objCash.getTxtnDate()));
			...
			else
			{
				...
				requiredDate=df.parse(sdf.format(c.getTime()));
		...
		{	
			logger.error("ERROR in PurchaseRequestViewModel ----> init", ex);			
		}
	}
...
//		}
	...
	@Command
	public void openItemsCommands(@BindingParam("type") PurchaseRequestGridData type)
	{
		//open items popup
		try
		{
		...
		arg.put("selectedPurchaseRequestGridDataRow",type);
		arg.put("lstItems", lstGridQBItems);
		//arg.put("type","items");
		...
		{
			logger.error("ERROR in PurchaseRequestViewModel  ----> openItemsCommands", ex);
		}
	}
	...
	public void refreshItemsParent(@BindingParam("selectedItem")QbListsModel selectedItem,@BindingParam("selectedPurchaseRequestGridDataRow")PurchaseRequestGridData selectedRow)
	{
		try
		{
			final PurchaseRequestGridData type=selectedRow;
					...
					if(compSetup.getPostOnMainClass().equals("Y"))
					{
						Messagebox.show("Selected Item have sub Sub Items(s). Do you want to continue?","Class", Messagebox.YES | Messagebox.NO  , Messagebox.QUESTION,
							...
							public void onEvent(Event evt) throws InterruptedException {
								...
								{	
									QbListsModel objItems=type.getSelectedItem();//data.getQbItemsData(type.getSelectedItem().getRecNo());
									if(objItems!=null)
									{
										...
										getNewTotalAmount();
										BindUtils.postNotifyChange(null, null, PurchaseRequestViewModel.this, "lstCheckItems");
										BindUtils.postNotifyChange(null, null, PurchaseRequestViewModel.this, "totalAmount");
									}
								}
								else 
								{		 
		...
		{
			logger.error("ERROR in PurchaseRequestViewModel  ----> refreshItemsParent", ex);
		}
	}
	/*********Contorl Events********************/
	...
	{	      
		...
		{			
			...
			if(result>0)
			{
				...
				if(purchaseRequestKey==0)
				{
					data.addUserActivity(data.GetNewUserActivityRecNo(), common.HbaEnum.HbaList.PurchaseRequest.getValue(), (int)obj.getRecNo(), obj.getMemo(), obj.getRefNUmber(), obj.getTxtnDate(),  webUserName, webUserID,common.HbaEnum.UserAction.Create.getValue());
				}else
				{
					data.addUserActivity(data.GetNewUserActivityRecNo(), common.HbaEnum.HbaList.PurchaseRequest.getValue(), (int)obj.getRecNo(), obj.getMemo(), obj.getRefNUmber(), obj.getTxtnDate(),  webUserName, webUserID,common.HbaEnum.UserAction.Edit.getValue());
				}
				
			}
			if(purchaseRequestKey>0)
			{
				Clients.showNotification("The Purchase Request Has Been Updated Successfully.",Clients.NOTIFICATION_TYPE_INFO, null, "middle_center", 10000,true);	   	        					        
			}
			else
			{
				Clients.showNotification("The Purchase Request Has Been Created Successfully.",Clients.NOTIFICATION_TYPE_INFO, null, "middle_center", 10000,true);		   		        					        
			}

		}catch (Exception ex) 
		{
			logger.error("ERROR in PurchaseRequestViewModel ----> Save", ex);			
		}
	}
...
//										BindUtils.postGlobalCommand(null, null, "resetGrid", args);
	...
	@Command
	@NotifyChange({"shipTo","address","lblExpenses","lblCheckItems","refNUmber","objCash","memo","selectedDropShipTo","selectedLstClass","itemReceiptKey","creationdate","requiredDate","labelStatus","selectedAccount","selectedPaytoOrder","lectedInvcCutomerSendVia","toatlAmount","lstCheckItems","lstExpenses","totalAmount","tempTotalAmount","actionTYpe","status","matchFlag","approveFlag"})
	public void navigationPurchaseRequest(@BindingParam("cmp") String navigation)
	{
		try
		{
			...
			if(objCash!=null && objCash.getRecNo()>0)
			{
				...
				totalAmount=objCash.getAmount();
				creationdate=df.parse(sdf.format(objCash.getTxtnDate()));
				requiredDate=df.parse(sdf.format(objCash.getReqDate()));
				//setSelectedPaytoOrder(selectedPaytoOrder);
				//setSelectedDropShipTo(selectedDropShipTo);
		...
		{	
			logger.error("ERROR in PurchaseRequestViewModel ----> navigationPurchaseRequest", ex);		
		}
	}
	...
	@Command
	@NotifyChange({"shipTo","address","lblExpenses","lblCheckItems","refNUmber","objCash","memo","selectedDropShipTo","selectedLstClass","itemReceiptKey","creationdate","labelStatus","selectedAccount","selectedPaytoOrder","lectedInvcCutomerSendVia","toatlAmount","lstCheckItems","lstExpenses","totalAmount","tempTotalAmount","actionTYpe","requiredDate"})
	public void copyFunctinality()
	{
		...
		{	
			logger.error("ERROR in PurchaseRequestViewModel ----> copyFunctinality", ex);		
		}
	}
	...
	public void goToRelatedReport()
	{
		try
		{
			...
			incContentPage.setSrc("/hba/report/purchaseRequestReport.zul");
		...
		{	
			logger.error("ERROR in PurchaseRequestViewModel ----> goToRelatedReport", ex);			
		}
	}
	...
	public void previewPdfForprintingInvoice()
	{
		...
		{	
			logger.error("ERROR in PurchaseRequestViewModel ----> previewPdfForprintingInvoice", ex);			
		}
	}
	...
	public void approveMaterialsRequest() {
		try {
			Map<String, Object> arg = new HashMap<String, Object>();
			Executions
			.createComponents("/hba/payments/approveMaterialsRequest.zul", null, arg);
		} catch (Exception ex) {
				...
				public void onEvent(Event evt)
					...
					if (evt.getName().equals("onNo")) {
						try {
							...
							logger.error("ERROR in PurchaseRequestViewModel ----> CustomerSendEmail",ex);
						}
					}
				}
			});
